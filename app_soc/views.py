...
from .models import WazuhFIMAlert, WazuhAgent, WebhookClient, WebhookAuthConfig, AccessFIM, AnalysisConfig, AnalysisResult, AutoStatusRule, FimDashboardGuide, FimDashboardGuideTranslation
from .pagination_utils import get_soc_table_page_size, SOC_TABLE_PAGE_SIZE_OPTIONS
...
# Import auto-status rule processor
from .auto_status_processor import apply_auto_status_rules
...
def get_user_accessible_companies(user):
    ...
    has_any_access_config = False
    
    for group in user_groups:
        try:
            access_fim = AccessFIM.objects.get(group=group)
            has_any_access_config = True
    ...
    # If user has no AccessFIM configuration at all, give access to all companies
    # This maintains backward compatibility for existing users
    if not has_any_access_config:
        return None
...
def check_fim_access(user, required_permission=None):
    ...
    # If user has no AccessFIM configuration at all, give basic access (view only)
    # This maintains backward compatibility for existing users
    if not has_any_access_config:
        # For specific permissions (edit/add/delete/configure), deny access if no config
        if required_permission in ['edit', 'add', 'delete', 'configure']:
            return False
        # For basic access (view), allow
        return True
...
def wazuh_fim_webhook(request):
    ...
    print("🔔 NEW FIM ALERT WEBHOOK REQUEST RECEIVED")
    print("=" * 80)
    print(f"📅 Time: {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"🌐 From IP: {request.META.get('REMOTE_ADDR', 'unknown')}")
    ...
    if request.method == 'OPTIONS':
        print("✅ CORS preflight request - responding with headers")
    ...
    if request.method != 'POST':
        print(f"❌ Invalid method: {request.method} (expected POST)")
        return JsonResponse({
            'success': False,
    ...
    try:
        ...
        if not file_name and file_path:
            ...
            print(f"   ✅ Extracted name from path: '{file_name}'")
        
        print(f"   📁 Final file_path: '{file_path}'")
        print(f"   📄 Final file_name: '{file_name}'")
        print(f"   📏 File size: {file_data.get('size_after') or file_data.get('size', 'unknown')}")
        ...
        print(f"🔍 DETERMINING ALERT TYPE:")
        print(f"   🎯 Syscheck event: '{syscheck_event}'")
        
        if syscheck_event in ['added', 'modified', 'deleted', 'read']:
            alert_type = syscheck_event
            print(f"   ✅ Alert type from syscheck event: '{alert_type}'")
        else:
            # Fallback to description parsing
            description = source_data.get('description', '').lower()
            print(f"   🔍 Fallback to description parsing: '{description}'")
            if 'added' in description:
                ...
                alert_type = 'read'
            print(f"   ✅ Alert type from description: '{alert_type}'")
        ...
        print(f"   💻 Platform: {agent_data.get('platform', 'unknown')}")
        print(f"   🖥️  OS: {agent_data.get('os', {}).get('name', 'unknown')}")
        ...
        if created:
            print(f"   ➕ Created new agent record for {agent_id}")
        else:
            print(f"   🔄 Updated existing agent record for {agent_id}")
        ...
        # Create FIM alert
        print("💾 CREATING FIM ALERT RECORD:")
        print(f"   🆔 Alert ID: '{alert_id}'")
        print(f"   📋 Rule ID: {rule_data.get('id', 0)}")
        print(f"   📝 Rule Name: '{rule_data.get('description', 'Unknown Rule')}'")
        print(f"   📊 Level: {rule_data.get('level', 5)}")
        print(f"   📁 File Path: '{file_path}'")
        print(f"   📄 File Name: '{file_name}'")
        print(f"   📏 File Size: {file_data.get('size_after') or file_data.get('size', 'unknown')}")
        print(f"   🎯 Alert Type: '{alert_type}'")
        print(f"   🤖 Agent: {agent_name} ({agent_id})")
        print(f"   👤 Client: {webhook_client.name if webhook_client else 'None'}")
        ...
        print(f"   ✅ Alert record created successfully in database!")
        print(f"   📋 Database ID: {fim_alert.id}")
        
        # Apply auto-status rules
        apply_auto_status_rules(fim_alert)
        
        logger.info(f"Created FIM alert {alert_id} for file {file_name}")
        
        # Cleanup old FIM alerts if needed
        try:
            from .models import FIMSettings
            settings = FIMSettings.get_settings()
            deleted_count = settings.cleanup_old_records()
            if deleted_count > 0:
                print(f"🗑️ Cleaned up {deleted_count} old FIM alerts")
                logger.info(f"Cleaned up {deleted_count} old FIM alerts (limit: {settings.max_records})")
        except Exception as e:
            print(f"⚠️ Error during FIM cleanup: {str(e)}")
            logger.error(f"Error during FIM cleanup: {str(e)}")
        
        # Send outgoing webhooks for the new alert
        try:
            # Determine event type based on alert level
            alert_level = rule_data.get('level', 5)
            if alert_level >= 10:  # Critical level (adjust as needed)
                event_type = 'critical_alert'
            elif alert_level >= 7:  # High level
                event_type = 'high_risk_alert'
            else:
                event_type = 'new_alert'

            webhook_result = send_fim_alert_webhooks(fim_alert, event_type)
            if webhook_result['sent'] > 0:
                print(f"📤 Sent {webhook_result['sent']} outgoing webhooks")
            if webhook_result['failed'] > 0:
                print(f"❌ Failed to send {webhook_result['failed']} webhooks")

        except Exception as e:
            print(f"⚠️ Error sending outgoing webhooks: {str(e)}")
            logger.error(f"Error sending outgoing webhooks for alert {alert_id}: {str(e)}")
...
def fim_alerts_dashboard(request):
    ...
    try:
        ...
        per_page = get_soc_table_page_size(request)
        ...
        context = {
            ...
            'page_size_options': SOC_TABLE_PAGE_SIZE_OPTIONS,
            'pagination_item_label': _('alerts'),
            'pagination_aria_label': _('Alerts pagination'),
        }
...
def fim_dashboard_guide(request):
    """Return JSON { content: html } for the FIM Dashboard guide (localized)."""
    ...
    try:
        ...
        country = None
    content = ''
    guide = FimDashboardGuide.objects.first()
    if guide:
        if country:
            trans = FimDashboardGuideTranslation.objects.filter(guide=guide, country=country).first()
            if trans and trans.content:
                content = trans.content
        if not content and guide.base_content:
            content = guide.base_content
        if not content:
            trans = FimDashboardGuideTranslation.objects.filter(guide=guide).select_related('country').first()
            if trans and trans.content:
                content = trans.content
...
@login_required
@require_POST
def fim_dashboard_guide_translate(request):
    """API for AI translation of FIM Dashboard guide content (admin)."""
    try:
        data = json.loads(request.body)
...
def mark_alert_processed(request, alert_id):
    ...
    try:
        ...
        alert.processed_by = request.user  # Set the user who processed the alert
        alert.save()
...
def clear_all_alerts_api(request):
    """API endpoint to clear all FIM alerts"""
    if request.method != 'POST':
        return JsonResponse({
            'success': False,
            'error': 'Method not allowed'
    ...
    try:
        ...
        if not check_fim_access(request.user, 'configure'):
            return JsonResponse({
                'success': False,
                'error': 'Permission denied: You need configure permission to clear alerts'
            }, status=403)
        
        # Count alerts before deletion
        alert_count = WazuhFIMAlert.objects.count()
        
        # Delete all alerts
        deleted_count, _ = WazuhFIMAlert.objects.all().delete()
        
        # Log the action
        logger.info(f"User {request.user.username} cleared {deleted_count} FIM alerts")
        
        return JsonResponse({
            'success': True,
            'deleted_count': deleted_count,
            'message': f'Successfully cleared {deleted_count} FIM alerts'
        })
    
    except Exception as e:
        logger.error(f"Error clearing FIM alerts: {str(e)}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
        return JsonResponse({
            'success': False,
...
def client_fim_webhook(request, client_id):
    ...
    """
    # Print incoming request information
    print("🔔 CLIENT FIM ALERT WEBHOOK REQUEST RECEIVED")
    print("=" * 80)
    print(f"📅 Time: {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def alert_process_api(request, alert_id):
    ...
    if request.method == 'POST':
        try:
            ...
            original_status = alert.processing_status
            original_risk = alert.risk_assessment
            ...
            # Always update processed_by and processed_at when status or risk assessment changes
            if (data.get('processing_status') and data.get('processing_status') != original_status) or \
               (data.get('risk_assessment') and data.get('risk_assessment') != original_risk):
                alert.processed_at = timezone.now()
...
from django.views.decorators.csrf import csrf_exempt
from django.contrib.auth.decorators import login_required
from .models import AnalysisConfig, FIMSettings
        ...
        try:
            ...
            for config in configs:
                configs_data.append({
                    ...
                    'data_type_display': config.get_data_type_display(),
...
@login_required 
@fim_access_required()
def analysis_configs_public_api(request):
    """API endpoint for getting public analysis configurations (for analysis selection)"""
    if request.method == 'GET':
        try:
            # Get only enabled analysis configurations (no configure permission needed)
            configs = AnalysisConfig.objects.filter(enabled=True).order_by('name')
            configs_data = []
            ...
            logger.error(f"Error getting public analysis configurations: {str(e)}")
            return JsonResponse({
                'success': False,
                'error': 'Failed to get analysis configurations'
            }, status=500)
...
@login_required
@fim_access_required('configure')
def analysis_config_create_api(request):
    """API endpoint for creating analysis configurations"""
    if request.method == 'POST':
        ...
        try:
            ...
            # Validate required fields
            required_fields = ['name', 'data_type', 'method', 'url']
            for field in required_fields:
