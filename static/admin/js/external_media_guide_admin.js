/**
 * External Media Guide admin: "Translate from base" for Guide translations (TinyMCE content).
 */
(function() {
    'use strict';
    ...
    function initTranslateButtons() {
        ...
        var $ = django.jQuery;
        var translateUrl = window.EXTERNAL_MEDIA_GUIDE_TRANSLATE_URL;
        ...
        $('.inline-related').each(function() {
            ...
            if (!contentId) return;
            if ($row.find('.external-media-guide-translate-btn').length) return;

            var $btn = $('<button type="button" class="external-media-guide-translate-btn button">')
