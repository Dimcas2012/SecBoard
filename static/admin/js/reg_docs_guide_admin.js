/**
 * Reg Docs (Document Registry) Guide admin: "Translate from base" for Guide translations (TinyMCE content).
 */
(function() {
    'use strict';
    ...
    function initTranslateButtons() {
        ...
        var $ = django.jQuery;
        var translateUrl = window.REG_DOCS_GUIDE_TRANSLATE_URL;
        ...
        $('.inline-related').each(function() {
            ...
            if (!contentId) return;
            if ($row.find('.reg-docs-guide-translate-btn').length) return;

            var $btn = $('<button type="button" class="reg-docs-guide-translate-btn button">')
