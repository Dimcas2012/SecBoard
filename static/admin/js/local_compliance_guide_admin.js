/**
 * Local Compliance Guide admin: "Translate from base" for Guide translations (TinyMCE content).
 */
(function() {
    'use strict';
    ...
    function initTranslateButtons() {
        ...
        var $ = django.jQuery;
        var translateUrl = window.LOCAL_COMPLIANCE_GUIDE_TRANSLATE_URL;
        ...
        $('.inline-related').each(function() {
            ...
            if (!contentId) return;
            if ($row.find('.local-compliance-guide-translate-btn').length) return;

            var $btn = $('<button type="button" class="local-compliance-guide-translate-btn button">')
