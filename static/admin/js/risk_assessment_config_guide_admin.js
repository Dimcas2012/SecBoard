/**
 * Risk Assessment Config Guide admin: "Translate from base" for Guide translations (TinyMCE content).
 */
(function() {
    'use strict';
    ...
    function initTranslateButtons() {
        ...
        var $ = django.jQuery;
        var translateUrl = window.RISK_ASSESSMENT_CONFIG_GUIDE_TRANSLATE_URL;
        ...
        $('.inline-related').each(function() {
            ...
            if (!contentId) return;
            if ($row.find('.risk-config-guide-translate-btn').length) return;

            var $btn = $('<button type="button" class="risk-config-guide-translate-btn button">')
