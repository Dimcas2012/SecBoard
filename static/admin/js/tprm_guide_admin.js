/**
 * TPRM Guide admin: "Translate from base" for Guide translations (TinyMCE content).
 */
(function() {
    'use strict';
    ...
    function initTranslateButtons() {
        ...
        var $ = django.jQuery;
        var translateUrl = window.TPRM_GUIDE_TRANSLATE_URL;
        ...
        $('.inline-related').each(function() {
            ...
            if (!contentId) return;
            if ($row.find('.tprm-guide-translate-btn').length) return;

            var $btn = $('<button type="button" class="tprm-guide-translate-btn button">')
