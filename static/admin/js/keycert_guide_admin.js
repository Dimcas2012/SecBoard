/**
 * Key/Cert Guide admin: "Translate from base" for Guide translations (TinyMCE content).
 */
(function() {
    'use strict';
    ...
    function initTranslateButtons() {
        ...
        var $ = django.jQuery;
        var translateUrl = window.KEYCERT_GUIDE_TRANSLATE_URL;
        ...
        $('.inline-related').each(function() {
            ...
            if (!contentId) return;
            if ($row.find('.keycert-guide-translate-btn').length) return;

            var $btn = $('<button type="button" class="keycert-guide-translate-btn button">')
