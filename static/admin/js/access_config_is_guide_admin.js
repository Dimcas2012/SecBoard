/**
 * Access Config IS Guide admin: "Translate from base" for Guide translations (TinyMCE content).
 */
(function() {
    'use strict';
    ...
    function initTranslateButtons() {
        ...
        var $ = django.jQuery;
        var translateUrl = window.ACCESS_CONFIG_IS_GUIDE_TRANSLATE_URL;
        ...
        $('.inline-related').each(function() {
            ...
            if (!contentId) return;
            if ($row.find('.access-config-is-guide-translate-btn').length) return;

            var $btn = $('<button type="button" class="access-config-is-guide-translate-btn button">')
