...
from .models import AccessDocs, MandatoryProcess, ProcessAttachment, ProcessExecution, ProcessEvidenceFile, ProcessFrequency, ProcessStatus, MandatoryProcessesGuide, MandatoryProcessesGuideTranslation
from .views import get_user_access_level, user_can_edit, get_mandatory_user_access_level, check_user_mandatory_edit_access, get_user_allowed_companies
    ...
    try:
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            try:
                ...
                # Map column index to field name (matching DataTables column order)
                order_columns = [
                    'process_name',           # 0: Process Name
                    'company__name',          # 1: Company
                    'description',            # 2: Description
                    'responsible_person__username',  # 3: Responsible Person
                    'additional_person__username',   # 4: Additional Person
                    'last_completed_date',    # 5: Last Completed
                    'frequency',              # 6: Frequency
                    'next_due_date',          # 7: Next Due Date
                    'priority',               # 8: Priority
                    'next_due_date',          # 9: Status (use next_due_date for sorting)
                    'source_document__name_doc',  # 10: Source Document
                    'created_at'              # 11: Attachment (use created_at for sorting)
                ]
        ...
        # Source Document list: only documents with Active status (filtered by company when company selected via AJAX)
        documents = RegisterDocs.objects.filter(
            groups__in=user_groups,
            status_doc__code__iexact='active'
        ).distinct().select_related('status_doc', 'company')
        ...
        if allowed_companies:
            if isinstance(allowed_companies, list):
                ...
                # Filter documents by allowed companies
        ...
        return render(request, 'app_doc/mandatory_processes.html', context)
...
def send_process_reminder(request):
    ...
    try:
        ...
        # Send emails
        from .email_utils import send_mandatory_process_reminder
    ...
    try:
        ...
        if export_all:
            ...
            processes = MandatoryProcess.objects.filter(
                ...
                'responsible_person', 'additional_person', 'groups', 'attachments'
            )
            ...
            if company_filter:
                ...
                processes = processes.filter(
                    Q(company__in=allowed_companies) | Q(company__isnull=True)
                )
        ...
        for row_num, process in enumerate(processes, 2):
            ...
            # Created By
            cell = ws.cell(row=row_num, column=16, value=process.created_by.get_full_name() if process.created_by else '')
            cell.font = data_font
            cell.alignment = cell_alignment
            cell.border = border
            
            # Created At
            cell = ws.cell(row=row_num, column=17, value=process.created_at.strftime('%Y-%m-%d %H:%M') if process.created_at else '')
            cell.font = data_font
            cell.alignment = cell_alignment
            cell.border = border
            
            # Updated By
            cell = ws.cell(row=row_num, column=18, value=process.updated_by.get_full_name() if process.updated_by else '')
            cell.font = data_font
            cell.alignment = cell_alignment
            cell.border = border
            
            # Updated At
            cell = ws.cell(row=row_num, column=19, value=process.updated_at.strftime('%Y-%m-%d %H:%M') if process.updated_at else '')
            cell.font = data_font
            cell.alignment = cell_alignment
            cell.border = border
