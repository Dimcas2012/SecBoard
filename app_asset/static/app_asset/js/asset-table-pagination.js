/**
 * Asset table pagination — identical markup/classes to app_access access_table_pagination.html
...
(function () {
    ...
    function applyAccessPaginationChrome(container) {
        container.className = 'access-table-pagination d-flex flex-wrap justify-content-between align-items-center gap-2 mt-3';
    }
        ...
        container.innerHTML =
            ...
            '<div class="d-flex flex-wrap align-items-center gap-3">' +
                '<div class="d-flex align-items-center page-size-selector">' +
                    ...
                    '</label>' +
                    '<select class="form-select form-select-sm access-table-page-size-select" ' +
                        'id="' + escapeHtml(selectId) + '" data-access-page-size-select ' +
    ...
    function bindPaginationEvents(container, handlers) {
        if (container._assetPaginationBound) {
            return;
        }
        container._assetPaginationBound = true;
        ...
        container.addEventListener('change', function (e) {
            if (!e.target.matches('[data-access-page-size-select]')) {
                return;
            }
    ...
    window.initAssetClientTablePagination = function (options) {
        ...
        function getMatchedRows() {
            return getDataRows().filter(function (tr) {
                return rowFilter.has(tr) ? rowFilter.get(tr) : true;
            });
        }
        ...
        function render() {
            ...
            getDataRows().forEach(function (tr) {
                if (!matchedSet.has(tr)) {
                    tr.style.display = 'none';
                    return;
                }
                var idx = matched.indexOf(tr);
                tr.style.display = (idx >= start && idx < end) ? '' : 'none';
            });

            var emptyRow = table.querySelector('tbody tr.empty-state');
            if (emptyRow) {
        ...
        return {
            setRowFilterMatch: function (tr, match) {
                rowFilter.set(tr, !!match);
            },
            resetPage: function () {
                currentPage = 1;
            },
    ...
    window.initAssetDataTablePagination = function (options) {
        var api = options.api;
        var container = document.getElementById(options.containerId);
        if (!api || !container) {
            return null;
