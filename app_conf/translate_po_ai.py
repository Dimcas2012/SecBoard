# SecBoard/app_conf/translate_po_ai.py
import os
import sys
import polib
import anthropic
...
sys.path.append(os.path.join(PROJECT_ROOT, 'SecBoard'))

# Manual configuration (fallback if Django setup fails)
MANUAL_CONFIG = {
    'api_key': 'your-api-key-here',  # Will be replaced by Django settings
...
def get_claude_settings():
    try:
        ...
        if settings:
            print("Successfully loaded Claude settings from Django")
            return settings
        else:
            print("No Claude settings found in database")
...
def remove_apostrophes(text):
    """Remove all apostrophes from the text."""
